...
term = wclick("Terminal", 40, 60)       # Terminal's window, where no program window covers it
steps = [*click(*DOCK["Terminal"]), wait_for("terminal: opened"),
         ...
         *click(*DOCK["Apps"]), wait_for("apps: opened"), *click(*DOCK["Clock"]), wait_for("apps: clock"),
...
echo "$out" | grep -qx "apps: calc started in slot 11" || fail "the dock's Calculator did not start calc through Apps"
echo "$out" | grep -qx "apps: clock started in slot 15" || fail "the dock's Clock did not start clock through the open Apps"
[ "$(echo "$out" | grep -c "^apps: opened")" = 1 ] || fail "Apps showed a window when the dock only asked it to start calc"
...
echo "ok: the tour sees every attack refused; snake, life and tiles run from Terminal, calc and clock from the dock, six at once, one copy of each"
