...
steps = [*click(512, 548), wait_for("terminal: opened"),
...
status=$?
echo "$out" | grep -E "^(tour|calc|snake|life|tiles|terminal: (run|ps|tour))" | sed 's/^/  | /'
...
echo "$out" | grep -qx "tour: Writing, then running code: Asked for write+execute on its data, got rw-" || fail "write+execute"
echo "$out" | grep -qE "^tour: Tampered programs: Checked now: [0-9]+ programs match the manifest, 0 refused$" || fail "tamper check"
...
echo "$out" | grep -qx "terminal: ps -> 10 running" || fail "ps did not count the running programs"
echo "$out" | grep -q "PANIC" && fail "kernel panicked"
echo "ok: the tour sees every attack refused; calc, snake, life and tiles run from the card, five at once"
