...
for who in alice display mallory input; do
  echo "$out" | grep -q "^leanos: $who verified, sha256 " || fail "$who was not verified"
done
echo "$out" | grep -q "^carol: " && fail "carol ran"
echo "$out" | grep -qx "display: boot checks shown: 4 verified, 1 refused" || fail "the boot screen did not show the refusal"
