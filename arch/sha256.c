/* SHA-256 (FIPS 180-4), to measure what each task is loaded with. Trusted, not proved:
   `make test` checks it against a known digest at every boot (see kmain). */
#include "arch.h"
...
void sha256_update(struct sha256 *s, const void *data, uint64_t n) {
    ...
    while (n--) {
        s->buf[s->fill++] = *p++;
        if (s->fill == 64) { block(s->h, s->buf); s->fill = 0; }
    }
}
